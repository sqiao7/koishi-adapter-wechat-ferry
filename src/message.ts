...
export class WechatFerryMessageEncoder<C extends Context = Context> extends MessageEncoder<C, WeChatFerryBot<C, WeChatFerryBot.Config>> {
  ...
  async flush(): Promise<void> {
    ...
    if (this.readySend.img) {
      ...
      if (res.data === false) this.bot.logger.error(`send image fail: ${this.readySend.img}`)
    }
    if (this.readySend.file) {
      const res = await this.bot.internal.image(this.readySend.file, this.channelId)
      this.bot.logger.info(res)
    }
  }
